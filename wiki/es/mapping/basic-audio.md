...
## Calidad de audio
Antes de comenzar el mapeo, asegúrese de tener un archivo fuente de alta calidad para su canción. Many maps have terrible audio files, often from YouTube rips or low bitrate files. This is a game for music after all, so having good audio is essential for a good player experience.
...
By purchasing and using a high quality audio file, not only are you supporting your artist, but you save yourself a lot of headache while mapping. Mira la diferencia de calidad para la misma canción, al mismo ritmo.
...
¿Puedes ver la diferencia? You cannot scale up audio quality; only starting with a high quality audio file can give you clean, percise lines.
...
To make it easier to map and make sure that the song is synced perfectly to the game's beat you need to set up your audio file correctly. Esta sección asumirá que estás utilizando [Audacity](https://www.audacityteam.org/).

### Planifica tu primera nota
Analiza la introducción de la canción. Depending on where in the song you want to place your first block, you'll need to avoid both a *hot start* (not enough time before the first block) and a too-long intro. Tu canción encajará en una de estas tres categorías:
...
:::warning NOTE on Variable BPM It is recommended that newer mappers choose a different song due to the increased difficulty associated in mapping variable BPM. ¿No estás seguro de si tu canción es variable? ¡Pásate por `#mapping-discussion` en BSMG y pregunta!
...
Esto es para confirmar que el BPM que ha encontrado en línea o manualmente coincide con el archivo de audio que tiene. This addition is temporary; you should remove the click track before [exporting your audio](#exporting).
