...
2. [Problemen na modding van het spel](#_2-game-issues-post-modding)
...
Als het spel bevroren is op het Health and Safety scherm, of je ziet een avatar T-posen zonder controle van het spel, [verifieer je bestanden](#verify-game-files-for-steam) als je het spel op Steam hebt of herinstalleer het spel op Oculus. Volg daarvoor de [Nieuwe installatie](#clean-installation) sectie.
...
:::: warning Hiermee worden je lokale scores en statistieken verwijderd. :::

#### 3.2 Hoe gebruik ik mod `x`?
Als je de Mod Assistant gebruikt, klik je op de mod en druk dan op de "Mod Info" knop. [beatmods](http://beatmods.com) heeft ook een "More Info" knop op elke mod.

#### 3.3 vibratie problemen
Gameplay Modifiers Plus had een schakelaar om controller vibratie in/uit te schakelen. Als je het hebt uitschakelt en daarna de mod hebt verwijderd, dan moet je het configuratiebestand waar het in staat wijzigen. Open `%appdata%\..\locallow\hyperbolic magnetisme\beat saber\settings.cfg` en zet `controllerRumbleEnabled` op `true`.
...
Zorg ervoor dat uw Custom Avatars plugin correct is geïnstalleerd en de nieuwste versie is, zorg er ook voor dat de afhankelijkheden er ook zijn. Je hebt mogelijk een beschadigde avatar, een beschadigde avatar kan al je andere avatars ook beschadigen. Hetzelfde geld voor levels en sabers.
...
**Plaats geen** nummers in de oude `Beat Saber\CustomSongs` map. Deze locatie is verouderd omdat het bestandsformaat voor custom levels is gewijzigd. Als je nummers in het oude formaat hebt (`.json` en `.ogg` bestanden in plaats van `.dat` en `.egg`), laat ze in de oude `Beat Saber/CustomSongs` map. Je moet ze opnieuw downloaden van BeatSaver.

Als alternatief kan je ze handmatig omzetten met [Song Converter](https://github.com/lolPants/songe-converter), maar je krijgt geen hulp bij deze methode en je moet het programma zelf compileren.
...
Als dit alleen gebeurt op bepaalde levels, missen ze mogelijk vereiste mods of zijn de bestanden kapot. Als het met al je nummers gebeurt, verwijder dan de `Plugins` map en installeer de mods opnieuw.
...
* Gebruik alsjeblieft de juiste kanalen, `#pc-help` voor pc mod support en `#quest-help` voor quest mod support. Ga naar `#pc-3d-modeling` en `#quest-3d-modeling` voor vragen over **het maken van je eigen avatars, platforms, blokken of sabers**, en `#mapping-discussion` voor vragen over **het maken van je eigen nummers**.
