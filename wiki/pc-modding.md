...
Either remove the `.dll` file from the `Plugins` folder, or click the `Uninstall` button in Mod Assistant or BSManager (In BSManager the `Uninstall` button is a trash can instead)
