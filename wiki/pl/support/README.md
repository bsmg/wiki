...
# Wsparcie
## Spis Treści
0. [Istotne kwestie](#0-important-notes)
1. [Problemy z instalatorem](#1-installer-issues)
2. [Problemy w grze](#2-game-issues-post-modding)
3. [Częste pytania](#3-common-questions)
4. [Rozwiązywanie problemów](#4-troubleshooting)
5. [Jeśli wszystko inne zawiedzie](#5-if-all-else-fails)
6. [Nadal mam problemy](#6-still-having-issues)

## 0. Aktualizacje
Po aktualizacji gry, na kanale `#server-announcements` na Discordzie BSMG powinny znajdować się najbardziej aktualne instrukcje dotyczące stanu modów. The following are more detailed instructions of the most common procedure.

#### Aktualizacja popsuła mi mody
**Uruchom grę raz** po aktualizacji. Następnie ponownie zainstaluj mody za pomocą instalatora znajdującego się w [poradniku dla początkujących](/pl/beginners-guide. md), na przykład Mod Assistant.

## 1. Brak modów?

#### 1.1 W nowej kopii gry nie pojawiają się żadne mody
Po pierwsze, upewnij się, że poniższe informacje są prawdziwe:
* **Uruchomiłeś grę przed zainstalowaniem modów**. BSIPA usuwa wszystkie mody przy pierwszym uruchomieniu gry po aktualizacji, aby zapobiec wczytywaniu niedziałających, starych modów do nowej wersji gry. Zainstaluj ponownie mody, jeśli tak się dzieje.
* Steam/Oculus uruchamia Beat Sabera z **tej samej instalacji**, w której znajdują się mody. *np. mody są na dysku D, ale Steam wczytuje grę z dysku C.* Ustaw poprawną lokalizację instalacji w twoim wybranym instalatorze.
* Jeśli instalowałeś mody ręcznie, upewnij się, że wszystkie pliki i pliki od nich zależne, które pobrałeś, umieściłeś w odpowiednich folderach.

#### 1.2 Miałem zainstalowane mody w starszej wersji, ale po aktualizacji żadne się nie wczytują

Jeżeli szczegóły zawarte w sekcji 1.1 powyżej się zgadzają, spróbuj po kolei poniższych rozwiązań.

**Rozwiązanie 1**
- Zaktualizuj BSIPA do najnowszej wersji (w ModAssistant lub ręcznie)
...
### Zweryfikuj pliki gry na Steam
Aby zweryfikować integralność plików gry, wykonaj następujące kroki:
1. Upewnij się, że aplikacja SteamVR jest zamknięta, ponieważ nie pozwoli ona na weryfikację plików gry w czasie swojego działania.
...
2. **Skasuj CAŁY folder Beat Sabera.** Ten sposób różni się od odinstalowania gry przez Steam, ponieważ w takim przypadku nadmiarowe pliki, które nie są częścią oryginalnej gry, nie zostaną usunięte.
> Steam:     ``\steamapps\common\Beat Saber\`
  Oculus:``\hyperbolic-magnetism-beat-saber\`

3. Ponownie zainstaluj grę przez sklep Steam lub Oculus
4. **Przed instalacją modów, uruchom grę**
5. Uruchom aplikację Mod Assistant, zainstaluj wybrane mody, po czym uruchom grę.
