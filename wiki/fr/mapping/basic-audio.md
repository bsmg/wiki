...
Pouvez-vous voir la différence? You cannot scale up audio quality; only starting with a high quality audio file can give you clean, precise lines.
...
Analysez l'intro de la musique. Depending on where in the song you want to place your first block, you'll need to avoid both a _hot start_ (not enough time before the first block) and a too-long intro. Ta musique rentrera dans l'une de ces trois catégories :
...
3. Click `Generate – > Silence...` ![Générer le silence...](/.assets/images/mapping/audacity-generate_silence.png)
...
4. Click `Generate – > Silence...` ![Générer le silence...](/.assets/images/mapping/audacity-generate_silence.png)
...
5. Copiez les autres paramètres recommandés ci-dessous: ![Menu Audacity Rhythm Track](/.assets/images/mapping/click-track-1.png)
...
3. Click `Generate menu – > Silence...` ![Générer le silence...](/.assets/images/mapping/audacity-generate_silence.png)
...
![Visualisation d'une chanson trop silencieuse](/.assets/images/mapping/louder.png)
...
3. As a starting point, copy the settings below and click OK. Make sure that `Compress based on Peaks` is checked. ![Comprendre la compression](/.assets/images/mapping/compression.png)
...
3. Enter the difference value you calculated earlier (it should be negative) into the `Amplification` field. `New Peak Amplification` field will repeat what the first input field says, this is normal for a song that is already peaking at 0db. ![Menu d'amplification](/.assets/images/mapping/amplify.png)
...
After the negative value Amplify effect your song will look something like this: ![Amplification effect](/.assets/images/mapping/bna_amplify.png)
